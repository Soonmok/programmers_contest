...
from tensorflow.keras.layers import Conv2D, MaxPool2D, concatenate, \
    ReLU, BatchNormalization, multiply, add, GlobalAveragePooling2D, Dense, Activation, Reshape
...
def stem_block(x, scope="stem", training=True):
    with tf.name_scope(scope):
        ...
        block_1 = Conv2D(filters=32, kernel_size=3, activation='relu')(x)
...
def inception_resnet_A(x, scope="inception_a", training=True):
    with tf.name_scope(scope):
        init_x = x
        split_conv_x1 = Conv2D(filters=32, kernel_size=1, activation='relu', padding='same')(x)

        split_conv_x2 = Conv2D(filters=32, kernel_size=1, activation='relu', padding='same')(x)
        split_conv_x2 = Conv2D(filters=32, kernel_size=3, activation='relu', padding='same')(split_conv_x2)

        split_conv_x3 = Conv2D(filters=32, kernel_size=1, activation='relu', padding='same')(x)
        split_conv_x3 = Conv2D(filters=48, kernel_size=3, activation='relu', padding='same')(split_conv_x3)
        split_conv_x3 = Conv2D(filters=64, kernel_size=3, activation='relu', padding='same')(split_conv_x3)
...
def inception_resnet_B(x, scope="inception_b", training=True):
    with tf.name_scope(scope):
        ...
        split_conv_x1 = Conv2D(filters=192, kernel_size=1, activation='relu', padding='same')(x)

        split_conv_x2 = Conv2D(filters=128, kernel_size=1, activation='relu', padding='same')(x)
        split_conv_x2 = Conv2D(filters=160, kernel_size=(7, 1), activation='relu', padding='same')(split_conv_x2)
        split_conv_x2 = Conv2D(filters=192, kernel_size=(1, 7), activation='relu', padding='same')(split_conv_x2)
...
def inception_resnet_C(x, scope="inception_c", training=True):
    with tf.name_scope(scope):
        ...
        split_conv_x2 = Conv2D(filters=192, kernel_size=1, activation='relu', padding='same')(x)
        split_conv_x2 = Conv2D(filters=224, kernel_size=(1, 3), activation='relu', padding='same')(split_conv_x2)
        split_conv_x2 = Conv2D(filters=256, kernel_size=(3, 1), activation='relu', padding='same')(split_conv_x2)
